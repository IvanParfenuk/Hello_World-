...
dict_pizzas = {'Добрыня': (open('Pizza-Dobrynya.jpg', 'rb'), text_about_dobrynya),
               ...
               'Чикен Бастони':(open('Chiken-Bastonni.jpg', 'rb'), text_about_chiken),
               'Салат с креветками':(open('Salat-Tsezar-s-krevetkami.jpg', 'rb'), text_about_salat)}
...
def send_text(message):
    ...
    if message.text == 'Горячие закуски🧆':
        bot.send_photo(message.chat.id, photo=dict_pizzas['Чикен Бастони'][0], caption=dict_pizzas['Чикен Бастони'][1], reply_markup=keyboard.markup2)
    elif message.text == 'Салаты🥗':
        bot.send_photo(message.chat.id, photo=dict_pizzas['Салат с креветками'][0], caption=dict_pizzas['Салат с креветками'][1], reply_markup=keyboard.markup2)
